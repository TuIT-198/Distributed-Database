...
def main():
    ...
    print("=" * 60)
    print("  ShardMasters — Kiểm tra khả năng chịu lỗi (Failure Test)")
    print("=" * 60)
    ...
    try:
        ...
        print(f"\n[1/7] Phân mảnh dữ liệu cho {NUM_NODES} node...")
        subprocess.run(
        ...
        print(f"[2/7] Khởi tạo database cho {NUM_NODES} node...")
        ...
        print(f"[3/7] Khởi động {NUM_NODES} node...")
        for i in range(NUM_NODES):
        ...
        print("[4/7] Chờ hệ thống sẵn sàng...")
        coordinator_url = f"http://localhost:{COORDINATOR_PORT}"
        if not wait_for_health(coordinator_url):
            print("[LỖI] Không thể khởi động hệ thống. Thoát.")
            return

        print("[OK] Tất cả node đã sẵn sàng!\n")

        # === Bước 2: Truy vấn baseline ===
        print("[5/7] Chạy truy vấn baseline (tất cả node hoạt động)...")
        ...
        # === Bước 3: Giết node 2 ===
        print(f"\n[6/7] Giết node {KILLED_NODE} (mô phỏng sự cố)...")
        ...
        print("[7/7] Chạy truy vấn sau khi node sập...")
        resp = requests.get(f"{coordinator_url}/aggregate", timeout=30)
        resp.raise_for_status()
        ...
        print(f"\n[LỖI] Bài kiểm tra thất bại: {e}")
        import traceback
        traceback.print_exc()

    finally:
        # Dọn dẹp tất cả tiến trình và file DB
        print("\n[DỌN DẸP] Dừng tất cả tiến trình...")
